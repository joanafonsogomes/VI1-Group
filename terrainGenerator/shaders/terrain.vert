...
uniform float maxHeight, noiseVariance, noiseScale, waterHeight;
...
float h(float x, float y){
    if(noiseVersion==0){
        ...
        float ret = max(waterHeight,fbmSmoother(vec3(x,0.0,y),H,lacunarity,octaves));
        return  ret;
    }
    else if(noiseVersion==2){
        return snoise(vec3(x/70,0.0,y/70))* maxHeight;
    }
